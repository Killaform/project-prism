...
import { 
    ...
    Rss, Globe2, AlertTriangle, AlertOctagon, Building2
} from 'lucide-react';
import { ResponsiveContainer, Tooltip, Legend } from 'recharts'; // Removed unused Bar, XAxis, YAxis, CartesianGrid
import './App.css';
...
const getSourceDisplay = (sourceTypeLabel) => {
  return sourceTypeDisplayMap[sourceTypeLabel] || { label: sourceTypeLabel || "Unknown", icon: sourceTypeDisplayMap.default.icon };
};
    ...
    <div className={`bg-white p-5 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out mb-6 border-l-4 ${
      ...
      <div className="flex items-start mb-2">
        ...
        <div className="flex-grow">
          <a href={result.link} target="_blank" rel="noopener noreferrer" className="text-xl font-semibold text-purple-700 hover:text-purple-900 visited:text-purple-800 hover:underline">
            {result.title || "Untitled"} <ExternalLink size={15} className="inline ml-1 text-gray-400 hover:text-purple-600" />
          </a>
      ...
      <div className="ml-9 mb-4 flex flex-wrap items-center text-xs text-gray-700 gap-x-5 gap-y-2">
        ...
        <span className="flex items-center truncate">
            {result.sentiment?.label === 'positive' ? <TrendingUp size={14} className="mr-1.5 text-green-500" /> : result.sentiment?.label === 'negative' ? <TrendingDown size={14} className="mr-1.5 text-red-500" /> : <HelpCircle size={14} className="mr-1.5 text-gray-500" />}
          Sentiment: 
          <span className={`font-semibold ml-1 ${ result.sentiment?.label === 'positive' ? 'text-green-600' : result.sentiment?.label === 'negative' ? 'text-red-600' : 'text-gray-900' }`}>
            {result.sentiment?.label ? result.sentiment.label.charAt(0).toUpperCase() + result.sentiment.label.slice(1) : 'N/A'} ({result.sentiment?.score?.toFixed(2) || '0.00'})
          </span>
        </span>
      </div>
      <div className="ml-9 pt-4 border-t border-gray-200">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div className="text-sm font-semibold text-gray-800" title="Intrinsic Credibility Score">
            Intrinsic Score: <span className="text-xl text-purple-700 font-bold">
...
const CredibilityOverviewCard = ({ displayedResults, activePerspectiveMode, allSearchResultsCount }) => {
  // ... (CredibilityOverviewCard remains identical to the last version you approved)
  if (!displayedResults) { 
    return ( <div className="bg-white p-6 rounded-xl shadow-lg text-center text-gray-500"> <HelpCircle size={32} className="mx-auto mb-2 text-purple-300" /> <p>Loading overview data...</p> </div> );
  }
  if (displayedResults.length === 0 && allSearchResultsCount === 0) { return null; }
  if (displayedResults.length === 0 && allSearchResultsCount > 0) {
    return ( <div className="bg-white p-6 rounded-xl shadow-lg text-center text-gray-500"> <HelpCircle size={32} className="mx-auto mb-2 text-purple-300" /> <p>No results match the current perspective filter.</p> </div>);
  }
    ...
    <div className="bg-white p-6 rounded-2xl shadow-xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-sm">
        <div className="bg-purple-50 p-5 rounded-xl shadow">
          <h4 className="font-bold text-purple-700 mb-2 text-base flex items-center"><Globe size={18} className="mr-2"/>Viewing Perspective</h4>
          <p className="text-gray-800 font-medium text-lg">{perspectiveLabels[activePerspectiveMode] || activeDisplayPerspective.replace(/^\w/, c => c.toUpperCase())}</p>
          <p className="text-gray-600 text-xs mt-1">Displaying {displayedResults.length} of {allSearchResultsCount} total fetched results.</p>
        </div>
        <div className="bg-indigo-50 p-5 rounded-xl shadow">
          <h4 className="font-bold text-indigo-700 mb-2 text-base flex items-center"><BarChart2 size={18} className="mr-2"/>Dominant Source Types</h4>
          {sortedSourceTypes.length > 0 ? (<ul className="list-none pl-0 text-gray-800 space-y-1">{sortedSourceTypes.map(([type, count]) => { const display = getSourceDisplay(type); return ( <li key={type} className="flex items-center justify-between py-0.5"><span className="flex items-center">{display.icon} {display.label}:</span> <span className="font-semibold bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded-full text-xs">{count}</span></li> ); })}</ul>) : <p className="text-gray-500">N/A</p>}
        </div>
        <div className="bg-pink-50 p-5 rounded-xl shadow">
          <h4 className="font-bold text-pink-700 mb-2 text-base flex items-center"> {predominantSentiment === 'Positive' ? <TrendingUp size={18} className="mr-2"/> : predominantSentiment === 'Negative' ? <TrendingDown size={18} className="mr-2"/> : <HelpCircle size={18} className="mr-2"/>} Sentiment Snapshot </h4>
          <p className="text-gray-800">Overall Lean: <span className="font-semibold text-lg">{predominantSentiment}</span></p>
          <p className="text-gray-600 text-xs mt-1">(Positive: {sentimentCounts.positive}, Negative: {sentimentCounts.negative}, Neutral: {sentimentCounts.neutral})</p>
        </div>
        <div className="md:col-span-2 lg:col-span-3 bg-green-50 p-5 rounded-xl shadow mt-2">
            <h4 className="font-bold text-green-800 mb-3 text-base flex items-center"><CheckCircle size={18} className="mr-2"/>Credibility & Fact-Checks <span className="text-xs font-normal text-gray-600 ml-2">(Current View)</span></h4>
            <div className="flex flex-wrap gap-x-8 gap-y-3 items-center">
                <p>Avg. Intrinsic Score: <span className="font-bold text-2xl text-green-700">{averageIntrinsicScore}</span></p>
...
function App() {
  ...
  const calculateIntrinsicScoreForItem = useCallback(async (item, itemIndexInAllResults) => {
    ...
    if (targetIndex === -1) { console.warn(`[FRONTEND] CalcScore: targetIndex -1 for item ${item.link}`); return item; } // Cannot find, return original
  ...
  }, [allSearchResults]); // Dependency on allSearchResults for findIndex
  ...
  const calculateAllIntrinsicScores = useCallback(async (resultsToScore) => {
    ...
    setAllSearchResults(currentMasterList => { // Update based on the initial list passed to this function
        return resultsToScore.map(originalItem => { // Iterate original list
            const updatedVersion = itemsWithScores.find(scoredItem => scoredItem.link === originalItem.link && scoredItem.perspective_query_type === originalItem.perspective_query_type);
            return updatedVersion || originalItem; // Merge
        });
    });
    setIsProcessing(false);
    return itemsWithScores; 
  }, []); // Removed dependency on allSearchResults to avoid loop with handleSearch if not careful
  ...
  useEffect(() => {
    ...
    console.log(`[FRONTEND] useEffect: Filtering/Sorting. Perspective: ${activeDisplayPerspective}, Total: ${allSearchResults.length}`);
    let filtered = [];
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (Array.isArray(rawBackendResults)) {
        ...
        // Pass these initial items to be scored and then set to allSearchResults
        const itemsWithScores = await calculateAllIntrinsicScores(initialItems); 
        setAllSearchResults(itemsWithScores); // This triggers the useEffect for filtering and display
  ...
  const getButtonClass = (mode) => `px-4 py-2 rounded-lg font-medium transition-all duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-opacity-50 ${activeDisplayPerspective === mode ? 'bg-purple-600 text-white shadow-lg scale-105' : 'bg-gray-200 text-gray-700 hover:bg-gray-300 hover:shadow-md'}`;
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-100 via-pink-100 to-indigo-100 text-gray-800">
      <header className="bg-white/80 backdrop-blur-md shadow-lg sticky top-0 z-50">
        <div className="container mx-auto px-6 py-3 flex items-center justify-between">
          <div className="flex items-center"> <img src={logoSrc} alt="Perspective Engine Logo" className="h-[225px] w-auto" /> </div> {/* CORRECTED LOGO SIZE */}
          <div> <button className="text-gray-600 hover:text-purple-700 p-2 rounded-full hover:bg-purple-100 transition-colors"> <Settings size={24} /> </button> </div>
        </div>
      </header>
            ...
            {displayedResults.map((result) => {
                ...
                return (
                  <SearchResultItem
                    ...
                    // onCalculateIntrinsicScore prop can be removed from SearchResultItem if not used by a button there
                  />
                );
              })}
          </div>
      ...
      <footer className="bg-gray-800 text-white text-center p-8 mt-16">
        <p className="text-lg">&copy; {new Date().getFullYear()} Perspective Engine</p>
